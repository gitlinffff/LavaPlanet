...
#
# Modified by linfel (linfel@umich.edu) on 7/26/2023. An additional argument
# parser for ordering the output records was added to this script. 
...
# This argument was added by linfel on 7/26/2023
parser.add_argument('-od', '--ordering-all',dest='ordering_all',
                    action='store_true', default=False,
                    help='This is an additional step for ordering all'
                         ' output records in an increasing wavenumber')
...
def get_parline(nu, S, A, Epp, gp, gpp):
    ...
    """
    Vp = Vpp = Qp = Qpp = ' '*15
    return '701%12.6f%10.3e%10.3e          %10.4f'\
...
# This section is utilized to sort the output of the original script in an increasing wavenumber order.
# This section was added by linfel on 7/26/2023.
if args.ordering_all:
    par_path = out_name
    # Read the records from the .par file
    with open(par_path, 'r') as file:
        records = file.readlines()
    # Sort the records based on the wavenumber column
    sorted_records = sorted(records, key=lambda x: float(x[3:15]))
    # Write the sorted records back to the .par file
    with open(par_path, 'w') as file:
        file.writelines(sorted_records)
    
    print(par_path, 'has been ordered in an increasing wavenumber.')
