...
heat_rate_profile = np.genfromtxt("/home/linfel/LavaPlanet/outputs/heat_rate_del.csv", delimiter=',')
...
dh = h_atm/nlayers
dx = 1
h, x = np.mgrid[dh:h_atm+dh:dh, 1:7+dx:dx]


plt.figure(figsize=(10, 6))
heatmap = plt.pcolor(x,h,heat_rate_profile,cmap='RdBu_r', vmin=-np.max(np.abs(heat_rate_profile)), vmax=np.max(np.abs(heat_rate_profile)))
