# method: grid scanning, using error of g to calculate SSE
...
# different B and S parameters for Malkmus model
B_list = generate_uneven_seq(-4,-2,2)#; B_list.reverse()
S_list = generate_uneven_seq(0.9,1.1,2)#; S_list.reverse()
...
for B in B_list:
    for S in S_list:
        ...
        SSE = 0
        for i in range(len(ck_k)):
            g_obsr = ck_g[i]
            g_pred = g_malkmus(ck_k[i],B,S)
            SSE = SSE + (g_obsr - g_pred)**2
        ...
        ite += 1
        print('B %.12f  S %.10f, B_optm %.12f  S_optm %.10f  SSE_min %f,  progress %.2f%%'%(B,S,B_optm,S_optm,SSE_min,ite/iteration*100))
...
k_model = generate_uneven_seq(np.floor(log_ck_k.min()),np.ceil(log_ck_k.max()),3)
